...
The bridge is the HTTP transport to a running Fusion Electronics session over Autodesk's "Fusion MCP Server" endpoint (`ping` / `eagle` / `read` / `stage-ulps` verbs). Step 0 of `docs/REVIEWER_INSTRUCTIONS.md` now runs on `tools/fusion_export.py` (next section), which imports this bridge; driving the ULPs over the bridge (`stage-ulps` + `RUN` dispatches) remains only as a fallback pattern. The ULP-over-bridge path was verified end-to-end 2026-08-28 against a live 1-sheet design before being superseded.

Non-obvious operational facts (learned the hard way — do not rediscover):

- **Dialog latch (known Fusion issue).** Two triggers are confirmed: any dispatch whose EAGLE command contains `RUN`, and any **no-op `EDIT`** (opening the sheet/editor that is already open — isolated by A/B test 2026-08-28). The add-in then refuses further scripts with "Cannot perform 'script' while a command dialog is open" — **no dialog is actually visible**; only a keypress (Escape) in the Fusion window clears it. Detect the clear by polling a no-op `execute_script`. Real editor switches, `RATSNEST`, `DISPLAY`, `WINDOW FIT`, and `EXPORT IMAGE` do not latch. `run_eagle()` auto-prefixes every command with `SET CONFIRM YES;`, which prevents the same latch after confirmation prompts and auto-answers ULP overwrite prompts — but prevents neither trigger above.
- ULP-fallback only: the schematic and board ULP passes **cannot merge into a single dispatch** — the images ULP terminates via `exit()`, which also ends the rest of the chained command line.
...
## Direct Capture (`tools/fusion_export.py`) — the Step 0 path

ULP-free capture: emits the same `-thomson-export-{sch,brd,stack}.json` contracts from `electronics_read` queries and renders the review PNGs via dispatched `EXPORT IMAGE` chains (verbs `sch` / `brd` / `images` / `all`). Verified against a same-day ULP capture of `series-shunt`: exact parity except documented deltas (`clearance_mm` null — MCP exposes no NetClass clearance; `layers_used` over-reporting; the stackup is *more* correct — the ULP's "ROUTE" name fallback put layer 19 Unrouted in the copper stack). Zero Escapes for 1-sheet designs; its `wait_channel()` prompts and waits if the latch arms. **The comet (routed + poured, round-outline) parity diff passed 2026-08-28** — every delta accounted for, several in the direct path's favor (the ULP was numerically blind to routing and pours on Fusion inner copper 303, and its edge-distance check measured against the drawing extent instead of the board outline). **Step 0 of `docs/REVIEWER_INSTRUCTIONS.md` now runs on this tool** (`python tools/fusion_export.py all`); the ULPs stay in `tools/` solely for the manual fallback path. Details and the intentional deltas: `docs/Direct_Read_Feasibility.md`.
