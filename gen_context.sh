...
# The tested driver is Claude Code, which reads ontology/, examples/, and the KB
# markdown directly and does NOT need this bundle (see README §5).
...
5.  **Comprehensive Review (run to completion):** Once you have the necessary information, perform the comprehensive design review based on the final instructions at the end of this document. Run all steps end-to-end — do NOT stop between steps to ask whether you should continue, dig into datasheets, or generate the report. The deliverable is the findings JSON plus the HTML report; the conversation is a thin wrapper around that deliverable, not the deliverable itself.
...
- Component centroids can be 5-10mm from the actual power pin on larger packages (SO8, TQFP, QFN), producing inflated distances and false positives.
